...
```{r}
library(tidyr) 
reviews_text<-separate(data = reviews_text, col = reviews_text, into = c("Sentiment", 
                              "SentimentText"), sep = 4)
```
...
reviews_text$SentimentText<-gsub("(?<=[\\s])\\s*|^\\s+|\\s+$", "", reviews_text$SentimentText, 
                                 perl=TRUE) 
...
```{r}
# Convert the cell values with a non-zero value to Y, and in case of a zero convert it to N,
